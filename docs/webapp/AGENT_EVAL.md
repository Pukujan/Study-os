...
The scorecard and embedded synthetic transcripts are written to `evals/out/player-a2a-scorecard.json`. They are evaluation artifacts only. The harness uses a throwaway database for the live API session rows and never inserts scorecards or synthetic learner evidence.

The tutor persona asks for the same idea shown another way, then the harness re-reads the served view. `capability_detector` requires that the reply carries a `regenerate_presentation` payload (`MISSING_REGENERATE_PRESENTATION` when absent or `null`), that the payload is a versioned `study-os.player-presentation.v1` update whose `version` is exactly one past the version that was served (`INVALID_REGENERATE_PRESENTATION`), that it keeps the current `step_id`/`concept_id`, that the served view actually adopted it (`RENDER_NOT_APPLIED`), and that the step, phase and variant did not move (`RENDER_MOVED_STEP`). A text-only reply therefore still fails the gate.

Each roleplay signs up a fresh learner with its own synthetic client IP, so a multi-seed run (`--seeds N`) does not exhaust the production per-IP signup and request budgets and report a spurious `rate_limited` instead of a product defect.
