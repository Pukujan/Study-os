...
| Decision layer | Rules → hosted **Jev** (grading `noul`/`choice`, misconception `choice` + `none_of_these`; eval-lab blind 89.87%; about $0.042/1M input) → frontier LLM on low confidence. **Laya-421M** self-hosted only for frustration/wants-answer/sentiment after calibration, never for grading. Next step is deterministic (rules + pyBKT + FSRS; bandit later). Day-one decision logging, 5% audit sampling, and switch criteria (fine-tune / harness / rule) come from DEEP_RESEARCH §7.5. |
