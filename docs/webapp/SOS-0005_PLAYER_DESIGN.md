...
| POST | `/api/player/sessions/{id}/tutor` | `{message}` (≤ 500 chars) | `{message_id, reply_md, prompt_version, model, served: "generated"|"fallback", suggested_action: "example"|"easier"|"harder"|"reexplain"|null, regenerate_presentation: PresentationUpdate|null}` (the client renders `suggested_action` as a button and never auto-applies it; an already-validated `regenerate_presentation` is applied **in place by the server** before the response is returned) |
...
`PlayerView = {session_id, lesson: {lesson_id, revision, title, lane, representation, total_steps}, step: {step_id, concept_id, index, teach_md, teach_frames, teach_collapsed: bool, probe: {prompt_md, frames, answer_kind, choices} | null, variant: int}, phase, presentation_version: int, presentation_update: PresentationUpdate | null, feedback: Feedback | null, scaffold, progress: {done, total}, is_guest: bool}`.

`PresentationUpdate` (`study-os.player-presentation.v1`) is the chat-triggered re-render contract. It carries the lesson/step/concept/variant/phase/scaffold identity it was generated for, `previous_version`/`version` (monotonic per session), the replacement `teach_md`/`teach_frames`, and `provenance` (`prompt_version`, `model`, `route`, `served`). It changes only the displayed teach content: step identity, probe, grading, phase, variant, scaffold and progress are untouched, and the version is assigned under the session row lock. A proposal that leaks a probe answer, claims mastery, exceeds the word budget, contains code fences, or names frames outside the step is rejected; the session keeps its previous presentation.
