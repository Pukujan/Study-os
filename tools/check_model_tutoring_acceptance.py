...
The gate evaluates learner-visible behavior and the decision trace. Corpus learner
signals may guide the simulated student, but they are never accepted as evidence for
progression. Progression must be justified by model-assessed learner evidence quoted
verbatim from the actual learner message.
"""
...
def evaluate_transcript(
    ...
    if len(rows) != expected_count:
        failures.append(_failure("TURN_COUNT", None, f"expected {expected_count} exchanges, found {len(rows)}"))
    ...
    for index, row in enumerate(rows):
        ...
        for phrase in INTERNAL_LEAKAGE:
            if phrase in lowered:
                failures.append(_failure("PRODUCT_INTERNAL_STATE_LEAK", index, f"contains {phrase!r}"))
        for alias in scenario.get("forbidden_aliases", []):
            if str(alias).lower() in lowered:
                failures.append(_failure("FORBIDDEN_VARIABLE_ALIAS", index, f"uses {alias!r}"))

        trace = trace_by_index.get(index)
        stage = None
        if trace is not None:
            stage = CONCEPT_TO_STAGE.get(str(trace.get("target_concept", "")))
        if stage is None and index < expected_count:
        ...
        if must_include_any and not any(str(term).lower() in lowered for term in must_include_any):
            failures.append(_failure("CALIBRATED_ANCHOR_MISSING", index, f"expected one of {must_include_any!r}"))
        for forbidden in expected.get("must_not_include", []):
            ...
            if forbidden_text and forbidden_text in lowered:
                failures.append(_failure("FUTURE_OR_FORBIDDEN_CONCEPT", index, f"contains {forbidden!r}"))
        if bool(expected.get("visual_required")) and not has_visual(teacher):
            failures.append(_failure("VISUAL_DROPPED", index, "calibrated stage requires a visual"))
        ...
        if isinstance(max_lines, int) and nonempty_line_count(teacher) > max_lines:
            failures.append(_failure("OUTPUT_BUDGET_EXCEEDED", index, f"{nonempty_line_count(teacher)} > {max_lines}"))
        if stage != "loop" and ("def " in lowered or "class " in lowered):
            failures.append(_failure("FULL_SOLUTION_LEAK", index, "implementation leaked before loop assembly"))
...
def evaluate_trace(
    ...
    if len(rows) != expected_count:
        failures.append(_failure("TRACE_TURN_COUNT", None, f"expected {expected_count} trace rows, found {len(rows)}"))
    ...
    for position, row in enumerate(rows):
        ...
        if missing:
            failures.append(_failure("TRACE_SCHEMA", index, f"missing required fields: {missing}"))
            continue
        if row.get("schema_version") != TRACE_SCHEMA_VERSION:
            failures.append(_failure("TRACE_SCHEMA_VERSION", index, f"expected {TRACE_SCHEMA_VERSION}"))
        if row.get("path_kind") != "model_generated":
            failures.append(_failure("NOT_MODEL_GENERATED", index, "path_kind must be model_generated"))

        concept = str(row.get("target_concept", ""))
        if concept not in CONCEPT_ORDER:
            failures.append(_failure("TARGET_CONCEPT_DRIFT", index, f"unsupported concept {concept!r}"))
            continue
        concept_index = CONCEPT_ORDER.index(concept)
        if position == 0 and concept_index != 0:
            failures.append(_failure("PROGRESSION_START", index, "pilot must begin at duplicate_meaning"))
        ...
        if assistance not in {"A0", "A1", "A2"}:
            failures.append(_failure("ASSISTANCE_CEILING", index, f"invalid assistance {assistance!r}"))

        outcome = row.get("learner_outcome")
        quote = row.get("evidence_quote")
        if outcome not in ALLOWED_OUTCOMES:
            failures.append(_failure("LEARNER_OUTCOME", index, f"invalid outcome {outcome!r}"))
        if not isinstance(quote, str):
            failures.append(_failure("EVIDENCE_QUOTE", index, "evidence_quote must be text"))
            quote = ""
        ...
        if quote and transcript and quote.casefold() not in learner_message.casefold():
            failures.append(_failure("EVIDENCE_NOT_IN_LEARNER_MESSAGE", index, "evidence_quote is not verbatim learner text"))
        if outcome == "demonstrated" and not quote.strip():
            failures.append(_failure("DEMONSTRATION_WITHOUT_EVIDENCE", index, "demonstrated requires a verbatim evidence quote"))
        advance = row.get("advance")
        if advance is True and outcome != "demonstrated":
            failures.append(_failure("UNSUPPORTED_ADVANCE", index, "advance requires demonstrated learner outcome"))
        if concept_index == len(CONCEPT_ORDER) - 1 and advance is True:
            failures.append(_failure("ADVANCE_PAST_FINAL_CONCEPT", index, "loop_assembly is final pilot concept"))
        ...
        if not isinstance(allowed_variables, list) or not all(isinstance(item, str) for item in allowed_variables):
            ...
            missing_variables = sorted(STAGE_REQUIRED_VARIABLES[stage].difference(allowed_variables))
            if missing_variables:
                failures.append(_failure("REQUIRED_VARIABLES_MISSING", index, f"missing {missing_variables}"))
        forbidden_variables = row.get("forbidden_variables")
        aliases = set(str(item) for item in scenario.get("forbidden_aliases", []))
        if not isinstance(forbidden_variables, list) or not aliases.issubset(set(forbidden_variables)):
            failures.append(_failure("FORBIDDEN_VARIABLE_POLICY_MISSING", index, f"must forbid {sorted(aliases)}"))
        if row.get("visual_required") is not True:
            failures.append(_failure("VISUAL_POLICY_DRIFT", index, "pilot stages require visuals"))
        for version_field in ("prompt_version", "model_identifier"):
            value = row.get(version_field)
            if not isinstance(value, str) or not value.strip():
                failures.append(_failure("TRACE_PROVENANCE", index, f"{version_field} must be nonempty"))
    ...
    if rows and str(rows[-1].get("target_concept")) != CONCEPT_ORDER[-1]:
        failures.append(_failure("PILOT_DID_NOT_REACH_LOOP", int(rows[-1].get("turn_index", -1)), "final concept must be loop_assembly"))
    return failures
...
def evaluate(
    *,
    ...
    scenario_id = str(scenario["id"])
    return {
        "schema_version": "study-os.model-tutoring-acceptance.v0.2",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Check bounded model tutoring against learner-visible and evidence contracts")
